// Poll for dependencies (robust for CodePen async loads)
function waitForDeps(cb, attempts=50) {
  if (window.THREE && window.Tweakpane && window.THREE.GLTFExporter && window.jspdf && window.SimplexNoise) {
    cb();
  } else if (attempts > 0) {
    setTimeout(() => waitForDeps(cb, attempts-1), 100);
  } else {
    console.error('Dependencies failed to load after 5s. Check external JS links.');
  }
}
...
async function init() {
  ...
  function buildRibbon(p) {
    const {turns, height, width, thickness, gridX, gridY, phase,
           superTwist, waveAmp, waveFreq, bulge, taper, noiseAmt, tilt, wireframe} = p;
    ...
    const mat = new THREE.MeshStandardMaterial({
      color:0x222222, metalness:0.9, roughness:0.2, wireframe
    });
  ...
  // ---------- Parameters ----------
  const params = {
    turns:2, height:4, width:1, thickness:0.06,
    gridX:40, gridY:120, phase:0,
    superTwist:0, waveAmp:0, waveFreq:2, bulge:0, taper:0,
    noiseAmt:0, seed:0, tilt:0, wireframe:true, baseSize:1.2
  };

  // ---------- Tweakpane UI ----------
  const pane = new Pane({container: document.getElementById('pane')});
  const f1 = pane.addFolder({title:'Core'});
  f1.addBinding(params,'turns',   {min:0, max:6, step:0.1});
  f1.addBinding(params,'height',  {min:1, max:10, step:0.1});
  f1.addBinding(params,'width',   {min:0.2, max:3, step:0.05});
  f1.addBinding(params,'thickness',{min:0, max:0.2, step:0.005});
  f1.addBinding(params,'gridX',   {min:10, max:80, step:1});
  f1.addBinding(params,'gridY',   {min:30, max:200, step:1});
  f1.addBinding(params,'phase',   {min:0, max:1, step:0.01});
  f1.addBinding(params,'tilt',    {min:-90, max:90, step:1});

  const f2 = pane.addFolder({title:'Shape Modulators'});
  f2.addBinding(params,'superTwist',{min:-3, max:3, step:0.1});
  f2.addBinding(params,'waveAmp',   {min:0, max:2, step:0.05});
  f2.addBinding(params,'waveFreq',  {min:0, max:10, step:0.1});
  f2.addBinding(params,'bulge',     {min:0, max:1, step:0.01});
  f2.addBinding(params,'taper',     {min:0, max:1, step:0.01});
  f2.addBinding(params,'noiseAmt',  {min:0, max:0.5, step:0.01});
  f2.addBinding(params,'seed',      {min:0, max:1000, step:1});

  const f3 = pane.addFolder({title:'Render'});
  f3.addBinding(params,'wireframe');
  f3.addBinding(params,'baseSize', {min:0.5, max:2, step:0.05});
  ...
  async function exportGLB() {
    ...
    const glb = await exporter.parseAsync(ribbon, {binary:true});
  ...
  async function exportPDF() {
    ...
    const views = [
      ...
      {azim:90,  elev:0,   label:'Left'},
      {azim:-90, elev:0,   label:'Right'},
      {azim:0,   elev:90,  label:'Top'},
      {azim:0,   elev:-90, label:'Bottom'}
    ];
    ...
    for (let i=0; i<views.length; i++) {
      ...
      pdf.setFontSize(10);
      pdf.text(v.label, x+size/2, y+size+8, {align:'center'});
    }
